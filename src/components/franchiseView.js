  ...
  teamNames.forEach((tn, rIdx) => {
    ...
    if (tn.dOhs > 0) {
      let tooltipList = (tn.dOhDetails || []).map(d => {
        ...
        return `
          <div class="py-1 px-1.5 rounded hover:bg-white/10 transition-colors flex items-center justify-between cursor-pointer group" onclick="event.stopPropagation(); window.jumpToMatchup(${tn.yr}, ${d.week}, '${safeOwner}')" title="Click to view ${tn.yr} Week ${d.week} Matchup Box Score">
